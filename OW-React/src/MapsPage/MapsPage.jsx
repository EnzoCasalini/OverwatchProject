...
function MapsPage() {
  ...
  const filteredMaps =
    filter === 'all' ? maps : maps.filter((map) => map.gamemodes.includes(filter))
  ...
  return (
    ...
    >
      ...
      <Grid container spacing={2} rowSpacing={2} alignItems="center" justifyContent="center">
        {filteredMaps.map((ow_map, index) => (
          <Grid item key={index}>
            <MapCard
              name={ow_map.name}
              screenshot={ow_map.screenshot}
              gamemodes={ow_map.gamemodes}
              location={ow_map.location}
              country_code={ow_map.country_code}
            />
          </Grid>
        ))}
      </Grid>
