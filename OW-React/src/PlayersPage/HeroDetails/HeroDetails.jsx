...
const HeroDetails = ({ playerInfo }) => {
  ...
  const heroStats =
    gameMode === 'quickplay'
      ? playerInfo.stats.pc.quickplay.heroes_comparisons.time_played
      : playerInfo.stats.pc.competitive.heroes_comparisons.time_played
  ...
  return (
    <Box sx={{ padding: '1rem', marginTop: '30px' }}>
      ...
      <ButtonGroup sx={{ display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}>
        <Button
          onClick={() => handleGameModeChange('quickplay')}
          variant={gameMode === 'quickplay' ? 'contained' : 'outlined'}
          color={'primary'}
        >
          Quick Play
        </Button>
        <Button
          ...
          variant={gameMode === 'competitive' ? 'contained' : 'outlined'}
          color={'primary'}
        >
          Competitive
        </Button>
          ...
          heroesList.map((hero, index) => (
            <Box key={index} sx={{ display: 'flex', alignItems: 'center', marginTop: '1rem' }}>
              ...
              <Typography sx={{ marginLeft: '1rem' }}>
                {(hero.value / 3600).toFixed(2)} hours
              </Typography>
            </Box>
          ))}
      </Box>
