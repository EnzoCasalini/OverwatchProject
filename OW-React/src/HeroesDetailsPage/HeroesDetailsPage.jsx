...
const HeroesDetailsPage = () => {
    ...
    const modifyHeroKey = (heroKey) => {
        if (heroKey.indexOf(': ') !== -1) {
            ...
            return heroKey.toLowerCase();
    }

    const getHero = async () => {
        setIsLoading(true);
        const response = await fetch(`https://overfast-api.tekrop.fr/heroes/${modifyHeroKey(heroKey)}?locale=${language}`);
    ...
    return (
        ...
        {isLoading ? (
            <Backdrop
                sx={{ color: '#f3f4ff', zIndex: (theme) => theme.zIndex.drawer + 10000 }}  open={true}>
                    ...
                    <CardContent sx={{display: "flex", alignItems: "center", flexDirection: "column", justifyContent: "space-evenly"}}>
                        ...
                        <Typography align="center" color="#f3f4ff" fontSize="24px" sx={{marginTop: "20px"}}>Skills : </Typography>
                        {
                            hero.abilities && hero.abilities.map((ability, index) => {
                                return (
                                    <HeroSkills key={index} ability={ability}/>
                                );
                            })
                        }
                    </CardContent>
